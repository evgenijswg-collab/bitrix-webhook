            ...
            for model in models:
                ...
                try:
                    ai_resp = requests.post(
                        ...
                        json={
                            "model": model,
                            "messages": [{"role": "user", "content": f"Напиши краткий вывод на русском (2-3 предложения): что хуже всего в этих данных, на что обратить внимание.\n\n{raw_text[:800]}"}],
                            "max_tokens": 300,
                            "temperature": 0.5
                        },
                        timeout=30
...
def run_monthly_audit():
    ...
    try:
        ...
        try:
                ...
                for item in items:
                    ...
                    if product_id not in product_names:
                        try:
                            ...
                            product_names[product_id] = p_resp.get('result', {}).get('product', {}).get('name', f'Товар {product_id}')
                        except:
                            product_names[product_id] = f'Товар {product_id}'
            ...
            if written_off:
                msgs.append(f"\n🗑 <b>Списано:</b>")
                for name, qty in sorted(written_off.items(), key=lambda x: x[1], reverse=True):
                    msgs.append(f"   • {name}: {qty:.1f} шт.")
            
            if not any([incoming, to_production, shipped, written_off]):
...
def ai_test():
    try:
        resp = requests.post(
            AI_API_URL,
            headers={"Authorization": f"Bearer {AI_API_KEY}", "Content-Type": "application/json"},
            json={"model": AI_MODEL, "messages": [{"role": "user", "content": "Скажи: тест пройден"}], "max_tokens": 50},
            timeout=30
        )
...
def telegram_webhook():
    try:
        ...
        if chat_id not in ALLOWED_CHAT_IDS:
            send_telegram_to_chat(chat_id, "⛔ У вас нет доступа к этой команде.")
